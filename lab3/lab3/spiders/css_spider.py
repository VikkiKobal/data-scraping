...
class CssSpider(scrapy.Spider):
    ...
    custom_settings = {
        ...
        'FEED_EXPORT_FIELDS': ['Країна', 'URL', 'Столиця', 'Офіційні мови', 'Незалежність', 'Валюта', 'Телефонний код', 'Прапор'],
        'IMAGES_STORE': 'flags_images',  
    }
    ...
    def parse_country_info(self, response):
        ...
        flag_url = response.css('#mw-content-text .infobox.geography img::attr(src)').get()
        if flag_url:
        ...
        yield {
            ...
            'Прапор': flag_url if flag_url else 'Невідомо',
