...
class CssSpider(scrapy.Spider):
    ...
    def parse(self, response):
        rows = response.css('table.wikitable tbody tr')[1:]  # Пропускаємо заголовок таблиці
    ...
    def parse_country_info(self, response):
        ...
        if infobox:
            ...
            for row in rows:
                header = row.css('th *::text').get()  # Витягуємо заголовок
                data = row.css('td *::text').getall()  # Витягуємо всі значення
